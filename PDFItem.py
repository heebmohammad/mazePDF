...
class PDFItem(FileItem):
    ...
    def getPreview(self, container)-> tk.Frame:
        return PDFItemFrame(container, self)
...
class PDFItemFrame(tk.Frame):
    ...
    def __init__(self, container, pdf_item: PDFItem):
        ...
        self.open_file_button = app_style.getItemControlButton(self, 
            "open pdf",
            app_style.getFileIconAssetKey(pdf_item.file_type), 
            self.pdf_item.openFile)
        self.open_file_button.grid(row=0, column=0, rowspan=3, sticky=tk.NS)

        # file name
        self.file_name_label = app_style.getStyleLabel(self, "name: " + pdf_item.getFileName(), True)
        self.file_name_label.grid(row=0, column=1, columnspan=2, sticky=tk.SW)

        # file size
        self.file_size_label = app_style.getStyleLabel(self, "size: " + pdf_item.getFormattedSize())
        self.file_size_label.grid(row=1, column=1, columnspan=2, sticky=tk.SW)

        # file number of pages
        self.num_pages_label = app_style.getStyleLabel(self, pdf_item.getFormmatedPagesNumber())
        self.num_pages_label.grid(row=2, column=1, columnspan=2, sticky=tk.NW)
